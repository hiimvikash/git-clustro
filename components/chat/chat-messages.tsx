...
interface ChatMessagesProps {
  name: string; // channelname or conversationName
  chatId: string; // channelId
  member: Member; // loggedIn member
  apiUrl: string; // /api/messages --- MessageGET
  socketUrl: string; // /api/socket/messages --- MessagePOST, PATCH AND DELETE
  socketQuery: Record<any, string>;
          ...
          <Fragment key={i}>
            {group.messages.map((message: MessageWithMemberWithProfile) => (
              <ChatItem
                key={message.id}
                id={message.id}
                currentMember={member} // loggedIn member
                member={message.member} // sender of particular msg
